...
def add_rolling_fraud_rate(df: pd.DataFrame, label_col='is_fraud', timestamp_col='timestamp', id_col='user_id', window=7):
    """
    Calculates the rolling fraud rate per user over a specific transaction window.
    
    Args:
        df: Input transaction DataFrame.
        label_col: Column name for the fraud target (0 or 1).
        timestamp_col: Column name for the transaction time.
        id_col: Column name for the unique user identifier.
        window: Number of previous transactions to include.
    """
